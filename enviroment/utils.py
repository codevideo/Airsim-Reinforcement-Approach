...
def World2BodyFrame(yaw, px, py, pz):
    yaw = -yaw
    rotation_matrix = np.array([[math.cos(yaw), -math.sin(yaw), 0],
                                [math.sin(yaw),  math.cos(yaw), 0],
                                [            0,              0, 1]])

    world_frame = np.array([[px],
                            [py],
                            [pz]])
    body_frame = np.dot(rotation_matrix, world_frame)
    return float(body_frame[0]), -float(body_frame[1]), -float(body_frame[2])

def clip_range(num, max, min):
    if num >= max :
        num = max
    elif num <= min :
        num = min
