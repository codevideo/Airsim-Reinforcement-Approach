...
for epoch in t_range:
    for i in range(ep_max_step):
        ...
        #print('original x shape',x.shape) #([batch, 1, 120, 160, 3])
        x_img = torch.squeeze(x/255).permute(-4,-1,-3,-2)
        #print('original x_img shape',x_img.shape) # batch,3,120,160
        ...
        reconst_loss = F.mse_loss(x_reconst, x_img, size_average=False)
    ...
    if (int(epoch) + 1) % 5 == 0: 
        torch.save(model.state_dict(), "dronet_full_convolution_model_folder/epoch"+str(epoch)+".pth")
        np.save('dronet_full_convolution_loss_folder/epoch_loss',epoch_loss)
...
#        x_concat = torch.cat([x.view(-1, 1, image_high, image_width), out.view(-1, 1, image_high, image_width)], dim=3)
        ...
        #x_ = torch.squeeze(torch.transpose(torch.transpose(x,-1,-2),-2,-3))
        #o_ = torch.transpose(torch.transpose(out,-1,-2),-2,-3)
        #print('x_.shape',x_.shape)
        #print('o_.shape',o_.shape)
#        x_concat = torch.cat([torch.squeeze(torch.transpose(torch.transpose(x,-1,-2),-2,-3)), torch.transpose(torch.transpose(out,-1,-2),-2,-3)], dim=3)
        x_concat = torch.cat([torch.squeeze(torch.transpose(torch.transpose(x,-1,-2),-2,-3)), out], dim=3)
